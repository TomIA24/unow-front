...
const Dialog = ({ onClose, quizId,index, setCurrentQuestionIndex,currentQuestion , isAnyAnswerSelected}) => {
  ...
  const fetchQuestions = async () => {
    try {
      const endpoint = `http://localhost:5050/api/quizapi/quiz/${quizId}/questions`;
      const allresponse = await axios.get(
        `http://localhost:5050/api/quizapi/quiz/${quizId}/questions`
      );
  ...
  const handleFlagQuestion = async (questionIndex) => {
    ...
    try {
      await axios.put(
        `http://localhost:5050/api/quizapi/updateQuiz/${quizId}/question/${questionId}`,
        {
